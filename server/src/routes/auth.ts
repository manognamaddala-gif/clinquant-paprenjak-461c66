...
router.post("/register", async (req, res) => {
  try {
    ...
    if (
      data.role === "authority" &&
      (
        !env.AUTHORITY_INVITE_CODE ||
        data.authorityCode !== env.AUTHORITY_INVITE_CODE
      )
    ) {
      return res.status(403).json({
        message: "Valid authority invite code required"
      });
    }
...
router.get("/me", auth, async (req: any, res) => {
  res.json({
    ...
    role: req.user.role,
    trustedContact: req.user.trustedContact || {}
  });
});
