...
router.post("/", auth, async (req, res) => {
    ...
    if (!consentedTracking)
        return res.status(400).json({ message: "Location tracking consent is required" });
...
router.patch("/:id/location", auth, async (req, res) => {
    ...
    if (![lat, lng].every(Number.isFinite))
        return res.status(400).json({ message: "Real location required" });
    const j = await Journey.findOneAndUpdate({ _id: req.params.id, userId: req.user._id, status: "active" }, { lastLocation: { lat, lng, accuracy, speed, heading, timestamp: new Date(timestamp || Date.now()) } }, { new: true });
    if (!j)
...
router.post("/:id/handshake", auth, async (req, res) => {
